 ...
 * factory
 *
 * @param {PassiveTree} passive_tree the passive tree that should be drawn
 * @param {string} query_selector dom selector for the wrapper.
 *                                should be a unique element
 *                                otherwise undefined result
 * @param {Object} user_conf configuration data
 * @return {Promise} ready when tree drawn
 */
module.exports = async function(passive_tree, query_selector, user_conf = {}) {
    ...
    if (tag_name == 'canvas') {
        ...
        throw new Error(`no PoeTreeDrawer implementation for ${tag_name}`);
    }
